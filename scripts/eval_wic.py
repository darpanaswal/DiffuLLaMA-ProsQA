#!/usr/bin/env python3
# eval_wic.py   (place in scripts/ of the cloned DiffuLLaMA repo)
"""
Evaluate a ddm-sft finetuned DiffuGPT-M on Word-in-Context (WiC).
...
attribution-backend imports — the same native generation the trainer relies on.
This is the whole point: if the repo can train the model, it can run inference,
so eval lives here next to training instead of in the DLIG repo.

Pipeline per item:
  1. prompt = WiC question (BOS + encoded + ====== sep, exactly like training).
  ...
  4. decode the generated suffix, read the first Yes/No after '###', compare to label.

WiC is balanced binary (same/different sense), chance = 50%. Reports overall
accuracy and per-class (same-sense / diff-sense) so the yes-bias is visible.
"""
...
def main():
    ...
    ap.add_argument("--data", default="LLaMA-Factory/data/wic_test_raw.jsonl",
                    help="raw WiC jsonl: {sentence1, sentence2, word, label} per line.")
    ...
    ap.add_argument("--gen_len", type=int, default=8,
                    help="masked generation region. WiC answer is just '### Yes.'/"
                         "'### No.' -> very short; 8 is plenty.")
    ...
    ap.add_argument("--out_file", default="wic_eval_preds.jsonl")
    ...
    def wic_prompt(ex):
        # MUST match training prompt (wic_to_diffusft.py build_prompt).
    ...
    for ex in data:
        q = wic_prompt(ex)
        # TRAINING FORMAT: <bos> prompt ====== ### <Yes/No>. <eos>
        # The "======" separator (sep_token_id 50155) sits between prompt and answer.
        # Must be reproduced or the model is off-distribution.
        prefix = [bos_id] + tokenizer.encode(q) + [tokenizer.sep_token_id]
        items.append({"q": q, "label": int(ex["label"]), "prefix": prefix})

    n, n_correct = 0, 0
    per_class = {0: [0, 0], 1: [0, 0]}   # label -> [correct, total]
    n_unreadable = 0
        ...
        for start in range(0, len(items), bs):
            ...
            for it, row, n_pad in zip(batch, res, pad_lens):
                ...
                fout.write(json.dumps({
                    "prompt": it["q"], "label": gold,
                    "gen": gen_text, "pred": pred, "ok": bool(ok),
                }) + "\n")

            print(f"  [{n}/{len(items)}] running acc: {100*n_correct/n:.1f}%", flush=True)
